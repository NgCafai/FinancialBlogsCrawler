...
class BlogsSpider(scrapy.Spider):
    ...
    def start_requests(self):
        for page in range(1, 88):
            url = 'http://blog.eastmoney.com/zhongqin/bloglist_0_%d.html' % page
    ...
    def parse_detail_page(self, response):
        ...
        content_list = response.css('#articleBody > font:nth-child(2)::text').extract() \
                       + response.css('#articleBody > p:nth-child(1) > font:nth-child(1)::text').extract() \
                       + response.css('#articleBody > p:nth-child(1)::text').extract()
        content = '\n'.join(
